...
gulp.task('prepareAPI', function(){
    ...
    if (site) {
        return gulp.src(variables.DEFAULT_API_JSON)
            ...
            .pipe(gulp.dest("./src/app/config/"+site+"/"))
            .pipe(gulpJson())
            .pipe(gulp.dest("./src/app/config/"+site+"/"));
        }
})
...
gulp.task('createLess', function() {
    ...
    if (site) {
        return gulp
            .src(['./src/app/config/'+site+'/theme.json'])
...
gulp.task('cleanConfigDir', function() {
  ...
  if (site && fs.exists('./src/app/config/'+site+'/')) {
    return gulp.src('./src/app/config/'+site+'/', {read: false})
        .pipe(clean());
  }
